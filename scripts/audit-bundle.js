...
if (!fs.existsSync(META_PATH)) {
  exitWithError('Missing dist/extension.meta.json. Run `npm run bundle` first.');
}
...
if (!outputMeta) {
  exitWithError('Metafile is missing output information for dist/extension.js.');
}
...
const dynamicEntries = outputImports.filter((imp) => imp.kind === 'dynamic-import' || imp.kind === 'require-resolve');
const unexpectedDynamic = dynamicEntries.filter((imp) => !ALLOWED_DYNAMIC.has(imp.path));
...
if (unexpectedExternals.length > 0) {
  ...
  errors.push('Unexpected externals detected (allowed externals are Node built-ins, vscode, @vscode/test-electron):');
...
if (Number.isFinite(bundleBytes) && bundleBytes > SIZE_BUDGET) {
  errors.push(`Bundle size ${formatBytes(bundleBytes)} exceeds budget ${formatBytes(SIZE_BUDGET)}.`);
}
...
console.log(`  size: ${formatBytes(bundleBytes)} (budget ${formatBytes(SIZE_BUDGET)})`);
console.log(`  externals: ${allowedExternalList.join(', ')}`);
if (dynamicEntries.length > 0) {
  const allowed = dynamicEntries.filter((imp) => ALLOWED_DYNAMIC.has(imp.path));
  if (allowed.length > 0) {
    console.log(`  dynamic requires (allowed): ${allowed.map((imp) => imp.path).join(', ')}`);
  }
}
...
function formatBytes(bytes) {
  ...
  const display = value >= 10 || value === Math.floor(value) ? value.toFixed(0) : value.toFixed(1);
